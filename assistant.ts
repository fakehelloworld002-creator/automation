...
let allDetectedIframes: Map<string, any> = new Map();  // Track all detected iframes: "name/id" → { metadata }
let iframeSnapshotBefore: Map<string, any> = new Map();  // Previous state to detect NEW iframes
let latestIframe: { id: string; name: string; detectedAt: number; isVisible: boolean } | null = null;  // Track the NEWEST iframe opened
let iframeCreationTimestamps: Map<string, number> = new Map();  // Track when each iframe was first detected: "iframe_id" → timestamp
 ...
 */
async function detectAllIframes(page: Page, windowPath: string = 'MAIN_PAGE'): Promise<Map<string, any>> {
    try {
 ...
 */
function logIframeMetadata(iframes: Map<string, any>, isNewDetection: boolean = false) {
    if (iframes.size === 0) {
    ...
    iframes.forEach((iframe) => {
        ...
        if (iframe.ariaLabel) log(`      │   AriaLabel: "${iframe.ariaLabel}"`);
 ...
 */
async function detectNewIframes(page: Page, windowPath: string = 'MAIN_PAGE'): Promise<Map<string, any>> {
    const currentIframes = await detectAllIframes(page, windowPath);
    const newIframes = new Map<string, any>();
    const now = Date.now();
    let newestIframe: { id: string; name: string; detectedAt: number; isVisible: boolean } | null = null;
    let newestTime = 0;
 ...
 */
function getIframesSortedByRecency(iframes: Map<string, any>): Array<any> {
 ...
 */
async function accessElementInIframe(
    page: Page,
    iframeIdentifier: string, // Can be iframe ID, name, or selector
    targetElement: string,     // Element text/selector to find
    action: 'click' | 'fill' | 'read',
    fillValue?: string
): Promise<{ success: boolean; foundIn: string; elementInfo: any }> {
    try {
        ...
        const elementSearchResult = await iframeLocator.evaluate((searchText: string) => {
 ...
 */
async function searchInAllDetectedIframes(
    page: Page,
    ...
    try {
        ...
        for (const iframe of iframesByRecency) {
            ...
            const result = await accessElementInIframe(
                page,
                iframe.id,
                targetElement,
                action,
 ...
 */
async function monitorIframeChanges(page: Page): Promise<{ newIframes: Map<string, any>; totalIframes: number }> {
    try {
