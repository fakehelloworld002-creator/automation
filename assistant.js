 ...
 */
async function searchInLaunchWinFrames(target, action, fillValue) {
    if (!state.page || state.page.isClosed())
        return false;
    try {
                ...
                if (action === 'click') {
                    ...
                    for (const elem of clickables) {
                        try {
                            ...
                            const visibleText = await elem.evaluate((el) => {
                    ...
                    if (exactMatches.length > 0) {
                        ...
                        if (!clickSuccess) {
                            try {
                                const jsClickResult = await match.elem.evaluate((el) => {
                                    try {
                                        el.click();
                                        return { success: true };
                                    }
