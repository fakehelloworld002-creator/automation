 ...
 */
async function detectNestedTabs(frame, windowPath) {
    try {
        const tabs = await frame.evaluate(() => {
            const detectedTabs = [];
            // Pattern 1: HTML <tab> or custom [role="tab"] elements
            const tabElements = Array.from(document.querySelectorAll('[role="tab"], [role="tablist"] > button, [role="tablist"] > div[role="tab"], .tab, .tabs > button, [data-tab], [aria-label*="Tab"]'));
            // Pattern 2: Tab container with nav structure
            const navTabs = Array.from(document.querySelectorAll('nav button, nav [role="button"], ul[role="tablist"] li, .nav-tabs li'));
            // Pattern 3: Bootstrap tabs pattern
            const bootstrapTabs = Array.from(document.querySelectorAll('.nav-tabs a, [role="presentation"] a'));
            // Pattern 4: Material Design tabs
            const mdTabs = Array.from(document.querySelectorAll('[role="tab"][aria-selected], .mat-tab-label'));
            const allTabLike = Array.from(new Set([...tabElements, ...navTabs, ...bootstrapTabs, ...mdTabs]));
            allTabLike.forEach((tab, idx) => {
                if (!tab.textContent || tab.textContent.trim().length === 0)
        ...
        const tabInfos = tabs.map((tab, idx) => ({
            tabName: tab.text,
 ...
 */
async function activateNestedTab(frame, tabName) {
    try {
        ...
        try {
            const verifyActive = await frame.evaluate((tabNameToCheck) => {
                ...
                const tabs = Array.from(document.querySelectorAll('[role="tab"], .nav-link, .tab-label, button[class*="tab"]'));
 ...
 */
async function detectVisibleModals(frame, windowPath) {
    try {
 ...
 */
async function searchWithTabPriority(frame, target, windowPath, action, fillValue) {
    try {
        ...
        if (detectedModals.length > 0) {
            ...
            try {
                const modalResult = await frame.evaluate((targetStr, actionStr, fillVal) => {
                    ...
                    if (actionStr === 'click') {
                        ...
                        const allElements = Array.from(topEl.querySelectorAll('*'));
        ...
        if (detectedTabs.length === 0) {
            ...
            if (action === 'click' && target.trim().toLowerCase() === 'ok') {
                ...
                let targetButton = okButtonInfo.find((btn) => btn.isOk);
                if (!targetButton) {
                    // FALLBACK: Contains "ok"
                    targetButton = okButtonInfo.find((btn) => btn.hasOkText);
                }
                if (targetButton) {
        ...
        const recursiveTabSearch = async (tabInfo, isActive) => {
 ...
 */
async function detectAndLogAllIframes() {
    if (!state.page || state.page.isClosed())
        return;
    try {
 ...
 */
async function savePageVideo(page, videoName) {
    if (page.isClosed()) {
        return '';
    }
    try {
 ...
 */
function formatResultsWorksheet(ws, rows) {
    if (!ws['!cols'])
        ws['!cols'] = [];
    if (!ws['!ref'])
        return;
    ...
    for (let R = range.s.r; R <= range.e.r; ++R) {
        for (let C = range.s.c; C <= range.e.c; ++C) {
            ...
            if (R === 0) {
                ...
                cell.s = {
                    alignment: { horizontal: alignment, vertical: 'top', wrapText },
 ...
 */
function generateConsolidatedReport(rows, excelFilePath) {
        ...
        const parentButton = await frame.evaluate((container) => {
            ...
            if (current.parentElement) {
                ...
                for (let i = containerIndex - 1; i >= Math.max(containerIndex - 3, 0); i--) {
                    ...
                    if (sibling.tagName === 'BUTTON' || sibling.getAttribute('role') === 'button' || (sibling.tagName === 'INPUT' && sibling.getAttribute('type') === 'button'))
                        return sibling;
                }
            }
            ...
            while (p && p !== document.documentElement) {
                if (p.tagName === 'BUTTON' || p.getAttribute('role') === 'button' || (p.tagName === 'INPUT' && p.getAttribute('type') === 'button'))
                    return p;
    ...
    try {
        ...
        if (targetTrimmedLower === 'ok') {
            ...
            if (okButtonFound.candidates && okButtonFound.candidates.length > 0) {
                ...
                okButtonFound.candidates.forEach((btn, idx) => {
                                ...
                                try {
                                    ...
                                    if (allText.includes(targetLower)) {
                                        ...
                                        try {
                                            ...
                                            try {
                                                await clickable.evaluate((el) => el.click ? el.click() : el.dispatchEvent(new MouseEvent('click', { bubbles: true })));
        ...
        try {
            ...
            const getCurrentVisibleDropdown = async () => {
                try {
...
async function clickWithRetry(target, maxRetries = 5) {
    ...
    const dropdownResult = await state.page?.evaluate((targetParam) => {
        ...
        for (const el of Array.from(interactiveElements)) {
            ...
            debugCount++;
            if (debugCount <= 15) {
                console.log(`   Match #${debugCount}: <${el.tagName}> type=${matchType} viewport=${isInViewport} depth=${dropdownDepth} text="${fullText.substring(0, 40)}"`);
            }
        }
    ...
    if (dropdownResult?.found) {
        ...
        log(`   Match type: ${dropdownResult.matchType} (EXACT/PHRASE/WORD_BOUNDARY/PARTIAL)`);
